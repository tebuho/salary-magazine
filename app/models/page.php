...
class page
{
    ...
    {
        $this->db->query(
            'SELECT msebenzi_onjani, ngowantoni, gama_le_company, job_title, province, province_slug, ndawoni, slug, id_yomntu, purpose, image,
             CASE
             ...
             END AS "closingDate"
             FROM imisebenzi
            WHERE imisebenzi.closing_date = "0000-00-00" AND timestampdiff(day, imisebenzi.created_at, now()) <= 7
            OR imisebenzi.closing_date >= :namhlanje
            ORDER BY imisebenzi.created_at DESC
            LIMIT :start, :rpp
         ');
        $this->db->bind(":namhlanje", date("Y-m-d"));
    ...
    {
        $this->db->query(
            'SELECT msebenzi_onjani, ngowantoni, gama_le_company, job_title, ndawoni, slug, id_yomntu, purpose, image,
             CASE
                WHEN DAYOFMONTH(`closing_date`) IN(1, 2, 3, 4, 5, 20, 22, 23, 24, 25, 29, 30, 31)
                THEN CONCAT(DATE_FORMAT(`closing_date`, "%D"), " ka ", DATE_FORMAT(`closing_date`, "%M %Y"))
                ELSE CONCAT(DATE_FORMAT(`closing_date`, "%e"), " ka ", DATE_FORMAT(`closing_date`, "%M %Y"))
    ...
    public function paginateImisebenziByType($data)
    {
        $this->db->query(
            ...
            WHERE ngowantoni = :ngowantoni
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            
            OR closing_date >= now() AND ngowantoni = :ngowantoni
            ORDER BY created_at DESC
            LIMIT :start, :rpp
         ');
        $this->db->bind(":namhlanje", date("Y-m-d"));
    ...
    public function getImisebenziByEmployer($employer)
    {
        $this->db->query(
            ...
            WHERE closing_date >=  now() AND employer_slug = :employer
            OR closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function paginateImisebenziByEmployer($data)
    {
        $this->db->query(
            ...
            OR closing_date >= now() AND gama_le_company = :employer
            ORDER BY created_at DESC
            LIMIT :start, :rpp
         ');
        $this->db->bind(":namhlanje", date("Y-m-d"));
    ...
    public function getImisebenziByExperience($exp)
    {
        $this->db->query(
            ...
            WHERE province = "Western Cape" AND closing_date >=  now() AND experience_slug = :experience
            OR province = "Western Cape" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function paginateImisebenziNgoBunjani($data)
    {
        $this->db->query(
            'SELECT msebenzi_onjani, ngowantoni, gama_le_company, job_title, ndawoni, slug, id_yomntu, purpose, image,
             ...
             END AS "closingDate"
             FROM imisebenzi
            WHERE msebenzi_onjani = :onjani
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            
            OR closing_date >= now() AND msebenzi_onjani = :onjani
            
            OR msebenzi_onjani = :onjani
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function paginateImisebenziNgeMfundo($data)
    {
        $this->db->query(
            'SELECT msebenzi_onjani, ngowantoni, gama_le_company, job_title, ndawoni, slug, id_yomntu, purpose, image,
             ...
             END AS "closingDate"
             FROM imisebenzi
            WHERE province = "Western Cape" AND mfundo = :mfundo
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR province = "Western Cape"
    ...
    public function searchImisebenzi($data)
    {
        $this->db->query(
            ...
            WHERE gama_le_company LIKE :search AND closing_date >=  CURDATE()
            OR province LIKE :search AND closing_date >=  CURDATE()
            OR ndawoni LIKE :search AND closing_date >=  CURDATE()
            OR ngowantoni LIKE :search AND closing_date >=  CURDATE()
            OR mfundo LIKE :search AND closing_date >=  CURDATE()
            OR ngowantoni LIKE :search AND closing_date >=  CURDATE()
            OR job_title LIKE :search AND closing_date >=  CURDATE()
            OR msebenzi_onjani LIKE :search AND closing_date >=  CURDATE()
            OR experience LIKE :search AND closing_date >=  CURDATE()
            OR purpose LIKE :search AND closing_date >=  CURDATE()
            OR requirements LIKE :search AND closing_date >=  CURDATE()
            OR skills_competencies LIKE :search AND closing_date >=  CURDATE()
            OR responsibilities LIKE :search AND closing_date >=  CURDATE()
            OR additional_info LIKE :search AND closing_date >=  CURDATE()

            -- Search for keyword without closing date
            OR gama_le_company LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR province LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR ndawoni LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR ngowantoni LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR mfundo LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR ngowantoni LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR job_title LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR msebenzi_onjani LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR experience LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR purpose LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR requirements LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR skills_competencies LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR responsibilities LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR additional_info LIKE :search AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
