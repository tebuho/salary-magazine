...
class page
{
    ...
    public function getImisebenzi()
    {
        $this->db->query(
            ...
            OR imisebenzi.province = "Western Cape"
    ...
    {
        $this->db->query(
            'SELECT job_type, category, gama_le_company, job_title, province, province_slug, ndawoni, slug, user_id, purpose, image,
             CASE
             ...
             END AS "closingDate"
             FROM imisebenzi
            WHERE imisebenzi.job_closing_date = "1970-01-01" AND timestampdiff(day, imisebenzi.created_at, now()) <= 7
            OR imisebenzi.job_closing_date >= :namhlanje
            ORDER BY imisebenzi.created_at DESC
            LIMIT :start, :rpp
         ');
        $this->db->bind(":namhlanje", date("Y-m-d"));
    ...
    {
        $this->db->query(
            '(SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "Eastern Cape" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "Eastern Cape" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
            UNION
            (SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "Western Cape" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "Western Cape" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
            UNION
            (SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "Gauteng" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "Gauteng" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
            UNION
            (SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "Free State" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "Free State" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
            UNION
            (SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "KwaZulu-Natal" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "KwaZulu-Natal" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
            UNION
            (SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "Limpopo" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "Limpopo" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
            UNION
            (SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "Mpumalanga" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "Mpumalanga" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
            UNION
            (SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "Northern Cape" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "Northern Cape" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
            UNION
            (SELECT `province`, `ndawoni`, `job_location_slug`, COUNT(*) AS count FROM `imisebenzi` WHERE `province` = "North West" AND `ndawoni` != "Various Areas" AND `job_closing_date` >=  now()
            OR `province` = "North West" AND `job_closing_date` = "1970-01-01" AND timestampdiff(day, `created_at`, now()) <= 7 GROUP BY `ndawoni` ORDER BY RAND() LIMIT 6)
         ');
        $results = $this->db->resultSet();
    ...
    public function getImisebenziByLocation($job_location)
    {
        $this->db->query(
            ...
            WHERE province = "Western Cape" AND job_closing_date >=  now() AND job_location_slug = :ndawoni
            OR province = "Western Cape" AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
    ...
    {
        $this->db->query(
            'SELECT job_type, category, gama_le_company, job_title, ndawoni, slug, user_id, purpose, image,
             CASE
                WHEN DAYOFMONTH(`job_closing_date`) IN(1, 2, 3, 4, 5, 20, 22, 23, 24, 25, 29, 30, 31)
                THEN CONCAT(DATE_FORMAT(`job_closing_date`, "%D"), " ka ", DATE_FORMAT(`job_closing_date`, "%M %Y"))
                ELSE CONCAT(DATE_FORMAT(`job_closing_date`, "%e"), " ka ", DATE_FORMAT(`job_closing_date`, "%M %Y"))
             END AS "closingDate"
             FROM imisebenzi
            WHERE province = "Western Cape" AND ndawoni = :ndawoni
            AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function filterImisebenziByType()
    {
        $this->db->query(
            ...
            OR job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
    ...
    {
        $this->db->query(
            'SELECT DISTINCT gama_le_company, province_slug, employer_slug, COUNT(*) AS count FROM imisebenzi
            ...
            GROUP BY gama_le_company ORDER BY count, gama_le_company LIMIT 36
         ');
        $results = $this->db->resultSet();
    ...
    public function paginateImisebenziByEmployer($data)
    {
        $this->db->query(
            ...
            WHERE gama_le_company = :employer
            AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            
            OR job_closing_date >= now() AND gama_le_company = :employer
            ORDER BY created_at DESC
            LIMIT :start, :rpp
         ');
        $this->db->bind(":namhlanje", date("Y-m-d"));
    ...
    public function filterImisebenziByExperience()
    {
        $this->db->query(
            ...
            WHERE province = "Western Cape" AND job_closing_date >=  now()
            OR province = "Western Cape" AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function getImisebenziByOnjani($onjani)
    {
        $this->db->query(
            ...
            OR province = job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            AND job_type_slug = :onjani
    ...
    public function getImisebenziByMfundo($education)
    {
        $this->db->query(
            ...
            WHERE province = "Western Cape" AND job_closing_date >=  now() AND job_education_slug = :job_education
            OR province = "Western Cape" AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function paginateImisebenziNgeMfundo($data)
    {
        $this->db->query(
            'SELECT job_type, category, gama_le_company, job_title, ndawoni, slug, user_id, purpose, image,
             ...
             END AS "closingDate"
             FROM imisebenzi
            WHERE province = "Western Cape" AND job_education = :job_education
            AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR province = "Western Cape"
    ...
    {
        $this->db->query(
            'SELECT gama_le_company FROM imisebenzi
            WHERE job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            ...
            OR job_closing_date >= :namhlanje AND province != "Not Applicable" AND province != "Nationwide"
            AND province != "Other - Non-South African Location"
            GROUP BY province ORDER BY gama_le_company ASC'
        );
        $this->db->bind(":namhlanje", date("Y-m-d"));
    ...
    public function searchImisebenzi($data)
    {
        $this->db->query(
            ...
            WHERE gama_le_company LIKE :search AND job_closing_date >=  CURDATE()
            OR province LIKE :search AND job_closing_date >=  CURDATE()
            OR ndawoni LIKE :search AND job_closing_date >=  CURDATE()
            ...
            OR gama_le_company LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR province LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR ndawoni LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR category LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR job_education LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR category LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR job_title LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR job_type LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR experience LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR purpose LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR job_requirements LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR skills_competencies LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR job_responsibilities LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
            OR additional_info LIKE :search AND job_closing_date = "1970-01-01" AND timestampdiff(day, created_at, now()) <= 7
