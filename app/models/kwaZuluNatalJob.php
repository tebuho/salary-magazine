...
class kwaZuluNatalJob
{
    ...
    public function filterImisebenziByLocation()
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND closing_date >=  now()
            OR province = "KwaZulu-Natal" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function getImisebenziByLocation($location)
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND closing_date >=  now() AND ndawoni_slug = :ndawoni
            OR province = "KwaZulu-Natal" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function getImisebenziByType($type)
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND closing_date >=  now() AND ngowantoni_slug = :ngowantoni
            OR province = "KwaZulu-Natal" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            AND ngowantoni_slug = :ngowantoni
            
            OR province = "Nationwide" AND closing_date >=  now() AND ngowantoni_slug = :ngowantoni
            OR province = "Nationwide" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            AND ngowantoni_slug = :ngowantoni
    ...
    public function paginateImisebenziByType($data)
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND ngowantoni = :ngowantoni
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function getImisebenziByEmployer($employer)
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND closing_date >=  now() AND employer_slug = :employer
            OR province = "KwaZulu-Natal" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function getImisebenziByExperience($exp)
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND closing_date >=  now() AND experience_slug = :experience
            OR province = "KwaZulu-Natal" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function getImisebenziByOnjani($onjani)
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND closing_date >=  now() AND onjani_slug = :onjani
            OR province = "KwaZulu-Natal" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            AND onjani_slug = :onjani
            
            OR province = "Nationwide" AND closing_date >=  now() AND onjani_slug = :onjani
            OR province = "Nationwide" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            AND onjani_slug = :onjani
    ...
    public function paginateImisebenziNgoBunjani($data)
    {
        $this->db->query(
            'SELECT msebenzi_onjani, ngowantoni, gama_le_company, job_title, province_slug, ndawoni, slug, id_yomntu, purpose, image,
             ...
             END AS "closingDate"
             FROM imisebenzi
            WHERE province = "KwaZulu-Natal" AND msebenzi_onjani = :onjani
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function getImisebenziByMfundo($education)
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND closing_date >=  now() AND mfundo_slug = :mfundo
            OR province = "KwaZulu-Natal" AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
    ...
    public function paginateImisebenziNgeMfundo($data)
    {
        $this->db->query(
            'SELECT msebenzi_onjani, ngowantoni, gama_le_company, job_title, province_slug, ndawoni, slug, id_yomntu, purpose, image,
             ...
             END AS "closingDate"
             FROM imisebenzi
            WHERE province = "KwaZulu-Natal" AND mfundo = :mfundo
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            OR province = "KwaZulu-Natal"
    ...
    public function searchImisebenzi($data)
    {
        $this->db->query(
            ...
            WHERE province = "KwaZulu-Natal" AND gama_le_company LIKE :search AND closing_date >=  CURDATE()
            
            OR province = "KwaZulu-Natal" AND gama_le_company LIKE :search
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            
            OR province = "KwaZulu-Natal" AND ndawoni LIKE :search AND closing_date >=  now() 
            
            OR province = "KwaZulu-Natal" AND ndawoni LIKE :search
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            
            OR province = "KwaZulu-Natal" AND job_title LIKE :search AND closing_date >=  now()
            
            OR province = "KwaZulu-Natal" AND job_title LIKE :search
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            
            OR province = "KwaZulu-Natal" AND ngowantoni LIKE :search AND closing_date >=  now()
            
            OR province = "KwaZulu-Natal" AND ngowantoni LIKE :search
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            
            OR province = "KwaZulu-Natal" AND mfundo LIKE :search AND closing_date >=  now()
            
            OR province = "KwaZulu-Natal" AND mfundo LIKE :search
            AND closing_date = "0000-00-00" AND timestampdiff(day, created_at, now()) <= 7
            
            OR province = "KwaZulu-Natal" AND responsibilities LIKE :search AND closing_date >=  now()
